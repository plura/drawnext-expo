// src/features/submission/steps/ReviewAndSubmitStep.jsx
...
export default function ReviewAndSubmitStep({ state, notebooks }) {
	const notebook = useMemo(
		() =>
			notebooks.find((n) => Number(n.id) === Number(state.notebookId)) ||
			null,
	...
	const neighborChips = useMemo(() => {
		...
		return state.neighbors
			...
			.map((n) => {
				...
				const label =
					sec?.label || (sec ? `Section ${sec.position}` : n.section_id)
	...
	return (
		<div className="space-y-4">
			...
			<div className="rounded-2xl border bg-white shadow-sm p-4">
				<dl className="grid grid-cols-[auto,1fr] items-center gap-x-3 gap-y-2 text-sm">
					...
					<dd className="font-medium">
						{notebook?.name || state.notebookId}
					</dd>
			...
			<p className="text-xs text-muted-foreground">
				Tap <span className="font-medium">Submit</span> below to send your
				drawing.
			</p>
		</div>
	)
